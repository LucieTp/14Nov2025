...
J'ai codé un petit jeu pour ton anniversaire (déso pour l'installation qui est un peu relou j'avoue, j'ai pas eu le temps de rendre le truc plus user friendly)
...
(4) Ouvrir le projet dans Pycharm (clique ouvrir > navigue vers la localisation de ton fichier dézippé - probablement "download/14Nov2025-main/" puis clique "ouvrir") - ça devrait t'ouvrir toute l'arborescence du projet)

La section un peu galère mtn - setup ton environnement et installe les modules:

(5) Sélectionne un interpreteur python: dans file > settings > Python > Interpreter clique sur "add interpreter" - une fenetre doit s'ouvrir. La dedans clique "generate new" et choisis la version python qu'il te propose, python 3.12 ou 3.13 normalement (nov 2025). Clique okay, ça devrait te créer un fichier .venv dans ton arborescence 

(6) Toujours dans pycharm, vas dans la section "terminal" (dans les boutons du menu vertical en bas à gauche) - ou alt + F12.
(j'espere que tu es tjrs sur windows pcq sinon ça marchera pas, faudra que tu te débrouilles)
...
Hopepefully ça marche and you enjoy
